...
public class InterserverFEWorker extends ServerWorker {
    ...
    private Logger logger = Logger.getLogger(InterserverFEWorker.class);
        ...
        if (packet.getOpCode() >= OpCodes.ISOpCodesStart) {
            switch (packet.getOpCode()) {
                case OpCodes.InterServer.ISMSG_HELLO: {
                    logger.info("Interserver " + ((HelloPacket) packet).getHello() + " sent hello to us!");
                }
                break;
                case OpCodes.InterServer.ISMSG_REG: {
                    ...
                    logger.info("Registering interserver for range " + startRange + " to " + endRange);
                ...
                case OpCodes.InterServer.ISMSG_BOOT_USER: {
                    logger.info("Booting char with UID = " + packet.getSender());
                ...
                case OpCodes.InterServer.ISMSG_USER_REG: {
                    ...
                    logger.info("Registering character with oldid = " + oldId + " and new id = " + packet.getSender());
            ...
            if (packet.getOpCode().equals(OpCodes.Server.Login.SMSG_LOGIN_FAILED)) {
                logger.info("User " + packet.getSender() + " failed to login");
                usersHolder.getUserChannel(packet.getSender()).write(packet);
            } else {
                if (packet.getSender().equals(0L)) {
                    //logger.info("Sending packet " + packet.getOpCode() + " to all players");
                    for (Long chan : usersHolder.getChannels())
                        try {
                            ...
                            if (c != null)
                                ...
                                logger.error("Channel for " + chan + " is not found");
                        } catch (Exception e) {
                    ...
                    try {
                        ...
                        if (c != null)
                            ...
                            logger.error("Channel for " + packet.getSender() + " is not found");
                    } catch (Exception e) {
                        logger.error("Unable to send message", e);
                    }
